...
async function getUserCount(serial) {
  ...
  const out = `${result.stdout}\n${result.stderr}`;
  // e.g. "UserInfo{0:Owner:c13}" / "UserInfo{10:Second Space:412}"
  const matches = [...out.matchAll(/UserInfo\{(\d+):([^:}]*)/g)];
  return {
    known: result.code === 0,
    count: matches.length,
    details: matches.map((m) => `id ${m[1]} (${m[2].trim() || "unnamed"})`),
    raw: out.trim(),
  };
}

// Extracts the individual accounts from `dumpsys account` output so failures can
// name the exact blocker, e.g. "user@gmail.com (com.google)". Any account of ANY
// type (Google, Mi, WhatsApp, ...) blocks dpm set-device-owner.
function parseAccountDetails(dumpsysOutput) {
  const details = [];
  const seen = new Set();
  const matches = [
    ...String(dumpsysOutput).matchAll(
      /Account\s*\{\s*name=([^,}]+),\s*type=([^,}]+)\s*\}/g,
    ),
  ];
  for (const m of matches) {
    const entry = `${m[1].trim()} (${m[2].trim()})`;
    if (!seen.has(entry)) {
      seen.add(entry);
      details.push(entry);
    }
...
async function getAccountCount(serial) {
  ...
  const details = parseAccountDetails(out);
...
function remediationStepsForBlockers(blockers, observed) {
  ...
  if (hasAccounts) {
    const list = (observed && observed.accountDetails) || [];
    if (list.length) {
      steps.push(`Blocking account(s) found on the device: ${list.join(", ")}.`);
      steps.push(
        "Remove each of them: Settings -> Accounts (Google/Mi account sign-out; " +
          "for app accounts like WhatsApp, uninstall the app).",
      );
    } else {
      steps.push(
        "Accounts are present on the device but names could not be read — " +
          "check Settings -> Accounts and remove all of them.",
      );
    }
  }

  if (hasUsers) {
    const list = (observed && observed.userDetails) || [];
    steps.push(
      `Extra Android users/profiles present${list.length ? `: ${list.join(", ")}` : ""}. ` +
        "Remove them (Second Space / work profile / guest) or run: adb shell pm remove-user <id>.",
    );
  }

  if (hasAccounts || hasUsers) {
    steps.push("Then reboot the phone and run Pre-check again.");
    steps.push(
      "If it still fails: factory reset, skip ALL sign-ins on first boot, " +
        "and run Install + Make Owner before adding any account.",
    );
  }
...
async function precheckDeviceOwner(serial, adminComponent) {
  ...
  // Log the concrete blockers so the UI log shows WHAT is blocking, not just counts.
  if (blockers.includes("accounts_present")) {
    sendLog(
      `[${serial}] Blocking account(s): ${
        observed.accountDetails.length
          ? observed.accountDetails.join(", ")
          : "present but unreadable — check Settings -> Accounts"
      }`,
      "warn",
    );
  }
  if (blockers.includes("multiple_users_present")) {
    sendLog(
      `[${serial}] Extra Android users/profiles: ${observed.userDetails.join(", ")}`,
      "warn",
    );
  }
  ...
  return {
    ...
    blockers,
    observed,
    steps: remediationStepsForBlockers(blockers, observed),
  };
}
...
async function installOnDevice(serial, adminComponent) {
  ...
  if (!ownerResult.ok) {
    if (
      ...
      // Enumerate what is actually blocking so the user doesn't have to guess.
      const [users, accounts] = await Promise.all([
        getUserCount(serial),
        getAccountCount(serial),
      ]);
      if (accounts.known && accounts.count > 0) {
        sendLog(
          `[${serial}] Blocking account(s): ${
            (accounts.details || []).length
              ? accounts.details.join(", ")
              : `${accounts.count} account(s) present but unreadable — check Settings -> Accounts`
          }`,
          "error",
        );
      }
      if (users.known && users.count > 1) {
        sendLog(
          `[${serial}] Extra Android users/profiles: ${(users.details || []).join(", ")}`,
          "error",
        );
      }
      const blockerSummary = [
        accounts.known && accounts.count > 0
          ? `accounts: ${(accounts.details || []).join(", ") || accounts.count}`
          : null,
        users.known && users.count > 1
          ? `users: ${(users.details || []).join(", ")}`
          : null,
      ]
        .filter(Boolean)
        .join(" | ");
      throw new Error(
        `Device owner blocked by Android policy (${blockerSummary || "accounts/users already present"}). ` +
          `Remove them, reboot, then retry. Raw: ${combined}`,
      );
    }
