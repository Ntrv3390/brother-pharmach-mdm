...
public interface SmsLogMapper {
    ...
    @Insert("INSERT INTO plugin_smslog_data " +
            "(deviceid, phonenumber, contactname, messagetype, messagebody, messagesimslot, smstimestamp, smsdate, createtime, customerid) " +
            "VALUES " +
            "(#{deviceId}, #{phoneNumber}, #{contactName}, #{messageType}, #{messageBody}, #{simSlot}, #{smsTimestamp}, #{smsDate}, #{createTime}, #{customerId})")
    ...
    @Insert({
        ...
        "#{item.messageBody}, #{item.simSlot}, #{item.smsTimestamp}, #{item.smsDate}, #{item.createTime}, #{item.customerId})",
        "</foreach>",
        "</script>"
    })
    ...
    @Select("SELECT id, deviceid AS deviceId, phonenumber AS phoneNumber, contactname AS contactName, " +
            "messagetype AS messageType, messagebody AS messageBody, messagesimslot AS simSlot, " +
    ...
    @Select({"<script>",
            ...
            "OR LOWER(messagebody) LIKE LOWER(CONCAT('%',#{search},'%'))) ",
            "</if>",
            "ORDER BY smstimestamp DESC ",
            "LIMIT #{limit} OFFSET #{offset}",
            "</script>"})
