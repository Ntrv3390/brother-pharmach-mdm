# SMS Log Plugin - API Testing Guide
...
curl -X GET "http://localhost:8080/rest/plugins/smslog/public/enabled/YOUR_DEVICE_NUMBER"
```

### 2. Submit Test SMS Logs (Android Device)

```bash
curl -X POST "http://localhost:8080/rest/plugins/smslog/public/submit/YOUR_DEVICE_NUMBER" \
...
### 3. Get SMS Logs (Admin - Requires Authentication)
...
curl -X GET "http://localhost:8080/rest/plugins/smslog/private/device/1?page=0&pageSize=50" \
...
```bash
curl -X GET "http://localhost:8080/rest/plugins/smslog/private/settings" \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl -X POST "http://localhost:8080/rest/plugins/smslog/private/settings" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "enabled": true,
...
### 6. Delete SMS Logs for a Device

```bash
curl -X DELETE "http://localhost:8080/rest/plugins/smslog/private/device/1" \
  -H "Authorization: Bearer $TOKEN"
```
...
SELECT * FROM plugins WHERE identifier = 'smslog';

-- Check call log data
SELECT * FROM plugin_smslog_data ORDER BY callTimestamp DESC LIMIT 10;

-- Check settings
SELECT * FROM plugin_smslog_settings;
...
**Solution:** 
- Check if plugin is built: `ls /home/mohammed/hmdm-server/plugins/smslog/core/target/`
- Check if WAR contains plugin: `jar -tf server/target/launcher.war | grep smslog`
- Check Tomcat logs: `tail -f /opt/tomcat9/logs/catalina.out`

### Issue: "Permission denied"
**Solution:**
- Ensure user has `plugin_smslog_access` permission
...
**Solution:**
- Verify data was submitted: `SELECT COUNT(*) FROM plugin_smslog_data;`
- Check customer ID matches: Device and call logs must have same customer ID
- Check if plugin is enabled: `SELECT * FROM plugin_smslog_settings;`
  ...
  "item": [
    ...
    },
    {
      ...
      "request": {
        ...
        "url": "http://localhost:8080/rest/plugins/smslog/public/submit/{{deviceNumber}}"
      }
    },
    {
      "name": "Get SMS Logs",
      "request": {
        ...
        "url": "http://localhost:8080/rest/plugins/smslog/private/device/{{deviceId}}"
      }
    }
  ]
}
...
### Load Test: Submit 1000 SMS Logs
...
for i in {1..1000}; do
  ...
  curl -s -X POST "http://localhost:8080/rest/plugins/smslog/public/submit/$DEVICE_NUMBER" \
    -H "Content-Type: application/json" \
    -d "[{
...
tail -f /opt/tomcat9/logs/catalina.out | grep -i smslog

# Check for errors
grep -i "error.*smslog" /opt/tomcat9/logs/catalina.out
```
...
-- Monitor table size
SELECT 
  pg_size_pretty(pg_total_relation_size('plugin_smslog_data')) as total_size,
